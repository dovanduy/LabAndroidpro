...
public class PCBluetoothSet extends Activity {
    public static int deviceType;
    String ip_ph;
    String ip_pc ;
//    String ip_tv = "192.168.1.100";
    ...
    List<DeviceList> pcConDevices= new ArrayList<DeviceList>();
    List<String> findDevices = new ArrayList<>();  //手机可连接已连接list
    List<DeviceList> conList = new ArrayList<DeviceList>();
    List<String> conListString = new ArrayList<>();
        ...
        public void handleMessage(Message msg) {
            switch (msg.what){
                ...
                case UPDATE_TEXT:
                    ...
                    templist1 = ChangeDevice.getInstance().removeDeviceString(deviceType,pcFindDevices,ctx);
                    templist = ChangeDevice.getInstance().changeList(templist1,ctx);
                    if(!templist.isEmpty()){
                        if (checkPC.isChecked()) {
                            pc_layout.setVisibility(View.VISIBLE);
                            listView.setVisibility(View.VISIBLE);
                        }
                        adapter = new DevicesAdapter(ctx, R.layout.devices_item, templist);
                        listView.setAdapter(adapter);
                        TextView txt = (TextView)findViewById(R.id.pcset_get_bluetoothdevices);
                ...
                case UPDATE_CONTEXT:
                    ...
                    templist = ChangeDevice.getInstance().removeDevice(deviceType,pcConDevices,ctx);
                    if (!templist.isEmpty()){
                        if (checkPC.isChecked()){
                            pc_conlayout.setVisibility(View.VISIBLE);
                            conListView.setVisibility(View.VISIBLE);
                        }
                        conAdapter = new ConDevicesAdapter(ctx, R.layout.condevices_item, templist);
                        conListView.setAdapter(conAdapter);
                        TextView txt = (TextView)findViewById(R.id.pcset_connected_bluetoothdevices);
                ...
                case UPDATE_FINDDEVICE:
                    ...
                    templist1 = ChangeDevice.getInstance().removeDeviceString(deviceType,list,ctx);
                    templist = ChangeDevice.getInstance().changeList(templist1,ctx);
                    if (templist.isEmpty()) {
                        TextView txt = (TextView)findViewById(R.id.pcset_ph_getDevices);
                ...
                /*case UPDATE_TV_FINDDEVICE:
                    ...
                    if (!tvFindDevices.isEmpty()){
                        ...
                        DevicesAdapter getAdapter1 = new DevicesAdapter(ctx, R.layout.devices_item, tvFindDevices);
                ...
                case UPDATE_TV_CONDEVICE:
                    ...
                    if (!tvConDevices.isEmpty()){
                        ...
                        DevicesAdapter adapter = new DevicesAdapter(ctx, R.layout.devices_item, tvConDevices);
    ...
    //type = 1则显示键鼠type = 2则显示音响type = 3显示所有checked设备
    public static void actionStart(Context context, int type){
        deviceType = type;
        Intent intent = new Intent(context, PCBluetoothSet.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pcbluetoothset);
        ...
        {
            /*Toast.makeText(PCBluetoothSet.this, "服务器已连接", Toast.LENGTH_SHORT)
                    .show();*/
            dataInputStream = DeviceConnection.getInstance().reInput();

        } else
        {
            /*Toast.makeText(PCBluetoothSet.this, "正在连接...", Toast.LENGTH_SHORT)
                    .show();*/
            doInBackground();
            dataInputStream = DeviceConnection.getInstance().reInput();
        }
    ...
    public void initListeners(){
        returnlogo = (ImageButton)findViewById(R.id.pcset_returnLogo_imgButton);
        ph_layout = (LinearLayout)findViewById(R.id.pcset_ph_layout);
        ph_conlayout = (LinearLayout)findViewById(R.id.pcset_ph_conlayout);
        pc_layout =(LinearLayout)findViewById(R.id.pcset_pc_layout);
        getListView = (ListViewForScrollView)findViewById(R.id.pcset_list_ph_getDevices);
        tvGetListView = (ListViewForScrollView)findViewById(R.id.pcset_list_tv_getDevices) ;
        tvConListView = (ListViewForScrollView)findViewById(R.id.pcset_list_tv_conDevices);
        listView= (ListViewForScrollView) findViewById(R.id.pcset_list_getDevices);
        conListView = (ListViewForScrollView) findViewById(R.id.pcset_list_conDevices);
        pc_conlayout =(LinearLayout)findViewById(R.id.pcset_pc_conlayout);
        tv_layout =(LinearLayout)findViewById(R.id.pcset_tv_layout);
        tv_conlayout =(LinearLayout)findViewById(R.id.pcset_tv_conlayout);
        refresh = (LinearLayout)findViewById(R.id.pcset_diskListRefreshLL);
        more = (LinearLayout)findViewById(R.id.pcset_diskListMoreLL);
        computerbluetooth = (LinearLayout)findViewById(R.id.pcset_computerbluetooth);
        bAdapter = BluetoothAdapter.getDefaultAdapter();
        final boolean swt_on =  bAdapter.isEnabled();
        swt_bluetooth = (Switch)findViewById(R.id.pcset_swc_bluetooth);
        checkPC = (CheckBox)findViewById(R.id.pcset_checkpc);
        checkPH = (CheckBox)findViewById(R.id.pcset_checkph);
        ...
        if (!checkPH.isChecked()) {
            ...
            ListViewForScrollView listView = (ListViewForScrollView) findViewById(R.id.pcset_list_ph_conDevices);
        ...
        checkPC.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean isChecked) {
                if (!isChecked){
                    pc_layout.setVisibility(View.GONE);
        ...
        conListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                final String s = pcConDevices.get(position).getIdName();
                String msg = "";
                msg= "是否与设备" + s + "断开连接？";
                showDailog(msg, new DialogInterface.OnClickListener() {
                    @Override
...
//        btn_devicesFresh =(Button) findViewById(R.id.pcset_btn_devicesfresh);
        //btn_conDevices =(Button) findViewById(R.id.pcset_connect_bluetoothdevices);
...
//                if((now.getTime()-timer)>3000){
                    ...
                    tv_conlayout.setVisibility(View.GONE);*/
//                    timer = now.getTime();
                    /*new GetConThread(ip_tv).start();//与tv连接获取设备列表
                    if (phSocket==null){
                        new Thread(new Runnable() {
                            @Override
                            public void run() {
                        ...
                        public void run() {
                            ...
                            if (ip_pc!=null){
                                try {
                                    ...
                                    getDevice.removeAll(getConDevice);
                                    Message msg = new Message();
...
//                bAdapter = BluetoothAdapter.getDefaultAdapter();
                    if (!bAdapter.isEnabled()){
                        ...
                        listView.setAdapter(adapter);
                        TextView txt = (TextView)findViewById(R.id.pcset_ph_conDevices);
                    ...
                    ctx.startActivity(intent);*/
                        ...
                        List<DeviceList> templist = ChangeDevice.getInstance().removeDevice(deviceType,conList,ctx);
                        ListViewForScrollView listView = (ListViewForScrollView)findViewById(R.id.pcset_list_ph_conDevices);
                        if (!templist.isEmpty()){
                            if (checkPH.isChecked()) {
                                ph_conlayout.setVisibility(View.VISIBLE);
